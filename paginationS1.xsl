...
<!--
    ...
    @knownBugs :    Positionnement de départ à déterminer
                    ...
                    Ne prend pas le contenu d'autres éléments que p.
...
<xsl:stylesheet 
    ...
    <!-- pagination -->
    <xsl:template match="p/text()|seg/text()" mode="phase1">
        ...
        <xsl:variable name="autoNum">
            <xsl:choose>
                <xsl:when test="($foliotation='foliotation') and ($num mod 2 != 1)">
                    <xsl:sequence
                        select="concat( format-number(($num div 2) - 16 , '0') , 'v' )" />
                </xsl:when>
                <xsl:when test="($foliotation='foliotation') and ($num mod 2 = 1)">
                    <xsl:sequence select="format-number( (( $num + 1 ) div 2) - 16 , '0' ) " />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:sequence select="format-number( ($num) - 16 , '0' )" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
            ...
            <xsl:attribute name="xml:id">
                <xsl:choose>
                    <xsl:when test="($foliotation='foliotation')">
                        <xsl:sequence
                            select="concat( $prefix , 'Fol' , $folio )" />
                    </xsl:when>
                    <xsl:otherwise>
