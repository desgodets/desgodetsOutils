...
<xsl:stylesheet 
    ...
    <!-- définition du motif -->
    <xsl:template match="front/@xml:id|body/@xml:id|back/@xml:id">
        <xsl:apply-templates />
    </xsl:template>
    <xsl:template match="div/@xml:id|titlePage/@xml:id|p[parent::div|parent::argument|parent::postscript|parent::epigraph]/@xml:id">
        <xsl:apply-templates />
    </xsl:template>
</xsl:stylesheet>