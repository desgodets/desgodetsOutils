...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <!-- régularisations -->
    <xsl:template match="p/text() | head/text() | hi/text() | seg/text()" mode="phase1">
        <xsl:analyze-string select="."
                    regex="\[\p{{Pi}}\p{{Z}}?(.*?)\p{{Z}}?\p{{Pf}}\p{{Z}}?;\p{{Z}}?(.*?)\]" flags="s">
    ...
    <!--<xsl:template match="hi[@rend='Desgodets_IndexLocorum']" mode="phase1">
        <xsl:analyze-string select="."
            regex="\[\p{{Pi}}\p{{Z}}?(.*?)\p{{Z}}?\p{{Pf}}\p{{Z}}?;\p{{Z}}?(.*?)\p{{Z}}?;\p{{Z}}?(.*?)\]" flags="s">
            <xsl:matching-substring>
    ...
    <xsl:template match="hi[@rend='Desgodets_IndexNominum']" mode="phase1">
        <xsl:analyze-string select="."
            regex="\[\p{{Pi}}\p{{Z}}?(.*?)\p{{Z}}?\p{{Pf}}\p{{Z}}?;\p{{Z}}?(.*?)\p{{Z}}?;\p{{Z}}?(.*?)\]" flags="s">
            <xsl:matching-substring>
    ...
    <xsl:template match="hi[@rend='Desgodets_GlossaireTechnique' or @rend='gT']" mode="phase2">
    ...
    <!--<xsl:template match="hi[@rend='Desgodets_GlossaireTechnique' or @rend='gT']" mode="phase2">
        <xsl:element name="term">
            ...
            <xsl:attribute name="ref">
                <xsl:value-of select="reg"/><!-\- ou ./reg ou child::reg -\->
            </xsl:attribute>
    ...
    <!-- glossaire juridique -->
    <!--<xsl:template match="hi[@rend='Desgodets_GlossaireJuridique' or @rend='gJ']" mode="phase2">
        <xsl:element name="term">
            <xsl:attribute name="type">
                <xsl:text>gJ</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="ref">
                <xsl:value-of select="reg"/><!-\- ou ./reg ou child::reg voir comment récupérer le texte normalisé -\->
            </xsl:attribute>
            <xsl:value-of select="orig"/>
        </xsl:element>
    </xsl:template>-->
    ...
    <!-- index rerum -->
    <!--<xsl:template match="hi[@rend='Desgodets_IndexRerum']" mode="phase2">
        <xsl:element name="term">
            <xsl:attribute name="type">
                <xsl:text>gT</xsl:text>
            </xsl:attribute>
    ...
    <!-- TODO sérialiser le traitement des trois index avec une règle nommée et un paramètre -->
